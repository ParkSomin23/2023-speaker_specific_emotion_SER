...
class Emotion_Network(nn.Module):
    def __init__(self, hparams):
        ...
        self.id_filter = nn.Parameter(torch.randn(1, 1+int(hparams['pool_head']), 768).float())
...
def main(hparam_path):
    ...
    load_weight = '/home/ubuntu/Dropbox/Using Speaker-specific Emotion Representations in wav2vec 2.0-based modules for SER/Vox_IEMO_REVISION/main_SamplerNoSampler/5_230530_2000/latest_model.pt'
    ww = torch.load(load_weight)
    kk = net.load_state_dict(ww['model_state_dict'], strict=True)
    ...
    optimizer.load_state_dict(ww['optimizer_state_dict'])
    scheduler.load_state_dict(ww['scheduler_state_dict'])
    ...
    try:
        best_acc = ww['best_acc']
        best_epoch = -1

        best_loss = 1e+5
        patience_check = 0
        
        for epoch in range(ww['epoch']+1, hparams['total_epochs']):  # loop over the dataset multiple times
